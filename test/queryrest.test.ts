...
describe('class QueryRest', () => {
  ...
  describe('queryChannelNames', () => {
    it('uses this.url', async () => {
      ...
      expect((qChNames.queryChannelNames as jest.Mock).mock.calls[0][0]).toEqual(DEFAULT_URL);
    });
    ...
    it('passes queryOptions', async () => {
      ...
      expect(qChNames.queryChannelNames).toBeCalledTimes(1);
      expect((qChNames.queryChannelNames as jest.Mock).mock.calls[0][1]).toEqual(options);
    });
  });
  ...
  describe('queryData', () => {
    ...
    it('passes queryRequest', async () => {
      ...
      const options: qData.QueryRequest = {
        channels: [{ backend: 'backend1', name: 'chan1' }, { backend: 'backend2', name: 'chan2' }],
        configFields: [qData.ConfigFields.GLOBAL_MILLIS, qData.ConfigFields.TYPE],
        eventFields: [
